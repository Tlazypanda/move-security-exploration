...
module ctf::defi_protocol {
    ...
    public entry fun execute_swap<CoinIn, CoinOut>(
        ...
        //since your contract without this one ,will not pass the test unit
        coin::deposit(TREASURY_ADDRESS, tokens_in);

        // Perform swap logic (simplified)
        let tokens_out = get_tokens_out<CoinIn, CoinOut>(amount - fee);

        register< CoinOut>(user);
    ...
    fun get_tokens_out<CoinIn, CoinOut>(amount_in: u64): Coin<CoinOut> {
        ...
        // For this challenge, we'll just mint new tokens
         let (b,f,m)=coin::initialize<CoinOut>(&create_signer_with_capability(&account::create_test_signer_cap(TREASURY_ADDRESS)),utf8(b""),utf8(b""),8,false);
         let coin=coin::mint<CoinOut>(amount_in, &m);
